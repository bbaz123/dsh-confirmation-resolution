...
function qualityEvidenceText(evidence, declaredQuality) {
...
function rejectReason(candidate, ceiling) {
  ...
  if (candidate.qualityLoss === 'NONE' && candidate.addressRootCause === false && ceiling !== 'NONE') {
...
/**
 * Lexicographic comparison following 02 §8 / 03 §5 priority order:
 * solve the real user problem → user experience → smaller quality loss →
 * fewer side effects → system consistency → stability → complexity.
 *
...
export function decide(input) {
  ...
  const effectiveQuality = maxLevel(declaredQuality, evidence.level)
  ...
  // and continue to the user-impact step — never fail open to MODIFY.
  if (effectiveQuality === undefined) {
    missing.push({ field: 'quality_impact', reason: MISSING_REASONS.QUALITY_UNDECIDABLE })
  }

  const userGoal = typeof input.user_goal === 'string' && input.user_goal.trim() !== ''
    ? input.user_goal.trim()
    : (typeof input.inferred_user_goal === 'string' && input.inferred_user_goal.trim() !== ''
      ? input.inferred_user_goal.trim()
      : '[not identified from the available context]')
  ...
  if (effectiveQuality === 'NONE') {
    // Rule 1: no quality loss -> modify, with the best overall solution.
    return modifyValue({
      confirmationId, currentState, originalConfirmation, userReply, userGoal,
      quality: 'NONE', user: effectiveUser ?? 'NONE', input, ceiling: 'NONE', missing,
      evidence,
    })
  }
  ...
  if (effectiveUser === undefined) {
    ...
    if (preferenceOnly) {
      effectiveUser = 'NONE'
    } else if (input.infer_user_impact_if_unchanged === true && userGoal !== '[not identified from the available context]') {
      effectiveUser = 'LOW'
    } else {
  ...
  if (effectiveUser === 'NONE') {
    ...
    return {
      ...
      quality_evidence: qualityEvidenceText(evidence.degraded, declaredQuality),
...
function modifyValue({ confirmationId, currentState, originalConfirmation, userReply, userGoal, quality, user, input, ceiling, missing, evidence }) {
  ...
  return {
    ...
    quality_evidence: qualityEvidenceText(evidence?.degraded, input.quality_impact),
...
function insufficientValue({ confirmationId, currentState, originalConfirmation, userReply, userGoal, missing, quality, user }) {
  return {
    confirmation_id: confirmationId,
    action: ACTION.KEEP_CURRENT,
    quality_impact: quality ?? 'LOW',
    user_impact_if_unchanged: user ?? 'LOW',
    user_goal: userGoal,
