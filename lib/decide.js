...
export const STATUS = Object.freeze({
  ...
  /** A ledger write succeeded (registration, or an explicit completion). */
  REGISTERED: 'REGISTERED',
  /** The guard refused the call: it cannot belong to a confirmation flow. */
...
export const MISSING_REASONS = Object.freeze({
  ...
  /**
   * 03 §9 makes goal identification the caller's job, and 02 §7 only has meaning
   * against a goal: selecting among solutions without one is guesswork. So a
   * MODIFY with no goal is refused rather than decided. KEEP_CURRENT needs no
   * goal, because nothing is being selected.
   */
  USER_GOAL_REQUIRED: 'USER_GOAL_REQUIRED_FOR_MODIFY',
})
...
export function decide(input) {
  ...
  if (effectiveQuality === 'NONE') {
    ...
    return modifyValue({
      confirmationId, currentState, originalConfirmation, userReply, userGoal, userGoalProvided,
...
function modifyValue({ confirmationId, currentState, originalConfirmation, userReply, userGoal, userGoalProvided, quality, user, input, ceiling, missing, evidence }) {
  // 03 §9 + 02 §7: choosing among candidate solutions is only meaningful against
  // a stated goal. Refuse a MODIFY that has none instead of picking blind.
  if (userGoalProvided === false) {
    return insufficientValue({
      confirmationId, currentState, originalConfirmation, userReply, userGoal,
      quality, user,
      missing: [...missing, { field: 'user_goal', reason: MISSING_REASONS.USER_GOAL_REQUIRED }],
    })
  }
  ...
  return {
    ...
    original_confirmation: originalConfirmation,
    user_reply: userReply,
    // A MODIFY decision is not the execution. The item stays PENDING here, and
    // only the `complete` action (after DSH reports success) resolves it — so a
    // failed execution is retryable instead of finding the item already closed.
