...
function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      candidate_solutions: {
        type: 'array',
        description: '候选方案列表。ACTION = MODIFY 时应给出至少一个替代方案，否则可能因缺少可行方案而返回 INSUFFICIENT_CONTEXT。每项字段：label、approach、solves_user_problem、quality_loss、side_effects、scope、reversibility、complexity、expands_beyond_necessity、violates_constraints、within_permitted_scope、addresses_root_cause、user_proposed。',
        items: { type: 'json' },
      },
      do_not_change: {
        type: 'string',
