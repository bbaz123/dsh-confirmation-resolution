 ...
 *
 * A C-number that is already RESOLVED may be re-registered with NEW text: a user
 * finishing a second task in one long session naturally numbers again from C1.
 * The ledger treats that as a new round and clears the previous decision, so an
 * earlier round's decision can never authorise the new one. Re-registering the
 * SAME text of a closed item stays a no-op (it is not a new round).
 */
function registerItem(ledger, itemId, args) {
  ...
  if (existing !== undefined
    ...
    // Identical text: the user re-published what was already closed. Registering
    // it again would resurrect a finished item, so refuse instead.
    return notApplicableValue({
      ...
      reason: `${itemId} is already RESOLVED with the same text; new text opens a new round`,
    })
  }
  ...
  const round = ledger.decisionOf(itemId)?.round ?? 1
  return ledgerValue({
    itemId,
    state,
    ledger,
    ...
    notes: `Confirmed: ${itemId} is now ${state}${round > 1 ? ` (round ${round})` : ''}. No decision was made. When the user replies to it, call again with action="decide".`,
  })
}
