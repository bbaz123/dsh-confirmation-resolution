...
每一步的状态变更都必须经过插件，DSH 不得在插件之外自行标记确认项状态：
    ...
    C1 = PENDING
        ↓ 用户回复 C1
        ↓ 2. decide(C1)                       → STATUS = READY_TO_EXECUTE，C1 仍为 PENDING
        ├── ACTION = KEEP_CURRENT  → decide 直接返回 C1 = RESOLVED（不修改本身已完成）
        └── ACTION = MODIFY        → DSH 实际执行修改
                                        ↓ 执行成功
                                        ↓ 3. complete(C1)   → C1 = RESOLVED
                                        ↓ 执行失败
                                     不调用 complete → C1 保持 PENDING，可重新 decide

- **MODIFY 决策不等于执行**：decide 返回 READY_TO_EXECUTE 后，确认项**仍是 PENDING**。只有 DSH 真正执行成功并调用 complete，该项才变成 RESOLVED。
- **执行失败时不要调用 complete**：保持 PENDING，修正后重新 decide，或重试执行。绝不能因为"已经决策过"就把该项当作已解决。
...
- ACTION = MODIFY：按 SELECTED_SOLUTION 执行，只修改 EXECUTION_SCOPE 指定的范围，不得修改 DO_NOT_CHANGE 指定的范围。**执行成功后调用 action="complete" 关闭该项**；执行失败则不调用 complete，该项保持 PENDING。
